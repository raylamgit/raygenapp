// GitLab mvpraygenapp group ray-raygenapp Ray 1/18/2023 10:46AM ADDED ZUNIT
...
// Agent labels
echo 'raygenapp Jenkinsfile pipeline started'
...
def srcGitBranch = 'createCompileUtil'
def zAppBuildGitRepo = 'https://github.com/raylamgit/mvpzapp.git'
def zAppBuildGitBranch = 'createCompileUtil'
...
def dbbHlq='RLAM'
def dbbBuildType='-i -v'
def dbbGroovyzOpts= '-DBB_PERSONAL_DAEMON -Dorg.slf4j.simpleLogger -cp /u/user3/logging'
...
def dbbCredentialOptions='-id RLAM -pf /var/dbb/conf/rlam.txt '



echo 'raygenapp Jenkinsfile pipeline Artifactory'
// Artifactory
...
//def ARTIFACTORY_REPO_PATH='hyc-mvp-team-maven-local/MVPraygenapp'
def ARTIFACTORY_REPO_PATH='hyc-mvp-team-maven-local/Priyaraygenapp'



// UCD
def ucdApplication = 'RayDemo_App'
def ucdProcess = 'Ray_Deploy_App_Bind_CICS_WEB_Proc'
...
echo 'raygenapp Jenkinsfile pipeline Stage Git Clone/Refresh '
pipeline {
...
//        RUN_IDZ_CODE_REVIEW=true
    ...
    //  DBB_URL='https://dbbsample-dbb-route-dbbns.apps.mvpopenshift.cp.fyre.ibm.com/dbb/'
        ...
        ARTIFACTORY_REPO_PATH='hyc-mvp-team-maven-local/MVPraygenapp'
//        ARTIFACTORY_REPO_PATH='hyc-mvp-team-maven-local/Priyaraygenapp'
...
echo 'raygenapp Jenkinsfile pipeline Check Env '
...
echo 'raygenapp Jenkinsfile pipeline dir -> raygenapp '
                    dir('raygenapp') {
                ...
                script{
                    ...
                    zUnitContents.each { zUnitContent ->
                        ...
                        sh (returnStatus: true, script: '''#!/bin/sh
                            curl --silent https://raw.githubusercontent.com/ibm/dbb-pipeline/main/raygenapp/zUnit/xsl/AZUZ2J30.xsl -o /tmp/AZUZ2J30.xsl
                ...
                script{
                    if ( hasBuildFiles ) {
                        ...
                        node( zOsAgentLabel ) {
                            ...
                            sh "$DBB_HOME/bin/groovyz ${WORKSPACE}/dbb/Pipeline/RunIDZCodeReview/RunCodeReview.groovy --workDir ${WORKSPACE}/BUILD-${BUILD_NUMBER}/${BUILD_OUTPUT_FOLDER} -cr  ${WORKSPACE}/raygenapp/raygenapp/cr-rules/CodeReviewRules.dat -ccr  ${WORKSPACE}/raygenapp/raygenapp/cr-rules/CodeReviewRules.ccr"
...
EXTREPO.url=${artiUrl}
 ...
 // password not encrypted, use my own
                         ...
                         //       sh "$dbbHome/bin/groovyz $dbbGroovyzOpts ${WORKSPACE}/dbb-new/Pipeline/CreateUCDComponentVersion/dbb-ucd-packaging.groovy --buztool ${ucdBuztool} --component ${ucdComponent} --workDir ${WORKSPACE}/BUILD-${BUILD_NUMBER}/${BUILD_OUTPUT_FOLDER} -prop /u/proy/Jenkins-Cloud/workspace/raygenappPipeline/BUILD-184/artifactoy.properties"
